...
float3 DiffuseLight(int index,float3 normal,float3 worldPos,float shadowAttenuation)
{
	...
	//点光源
	float3 lightVector = lightPositionOrDirection.xyz - worldPos;
...
VertexOutput LitPassVertex (VertexInput input) {
	...
	for (int j = 4; j < min(unity_LightIndicesOffsetAndCount.y, 8); j++) {
		int lightIndex = unity_4LightIndices1[j - 4];
